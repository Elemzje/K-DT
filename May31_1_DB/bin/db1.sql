...
--		참조 받는 테이블의 값이 변경될 때, 참조하는 테이블의 제약을 받는 것
-----------------------------------------------------------------------------
...
--		참조하는 테이블은 참조받는 테이블의 PK나 Unique로 지정된 컬럼만을
--		FK로 지정할 수 있음
...
create table major(
	...
	constraint pk_m primary key(m_code)
);

insert into major values(1001, '컴퓨터공학과');
insert into major values(1002, '물리학과');
insert into major values(1003, '화학과');
insert into major values(1004, '화학공학과');
select * from major;
...
insert into student values(101, '김학생', 1001);
insert into student values(201, '최학생', 1002);
insert into student values(301, '박학생', 1003);
insert into student values(401, '이학생', 1004);
insert into student values(201, '마학생', 2004); -- major 테이블에는 2004 코드 X
select * from student;
drop table student cascade constraint purge;
-- 어떤 특정학과가 없어지면 이 학생의 정보도 지워지게
delete from major where m_code = 1001;
-- major테이블에서 1001코드를 지우면, student테이블에서 1001코드를 가지고 있던
--	모든 학생의 정보가 지워짐
------------------------------------------------------------------------------
-- 5. Check : 데이터의 값의 범위나 조건을 설정해서
--				조건에 해당하는 데이터만 입력을 허용
create table example8(
	col1 number(3) constraint ex_check check(col1 between 1 and 9)
);

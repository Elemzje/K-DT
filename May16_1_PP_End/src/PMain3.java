...
//	nextLine()는 한줄 단위로 입력을 받기 때문에 개행문자(Enter)도 포함됨
...
public class PMain3 {
	public static void main(String[] args) {
		...
		double timeDiff = Math.abs(Math.abs(result1 - 10)-Math.abs(result2 - 10));
		
		if (Math.abs(result1-10)<Math.abs(result2-10)) {
			System.out.printf("%s님이 %.3f초 차이로 승리하셨습니다 !", user1, timeDiff);
		} else if (Math.abs(result1-10)>Math.abs(result2-10)) {
			System.out.printf("%s님이 %.3f초 차이로 승리하셨습니다 !", user2, timeDiff);
		} else {

...
select * from jun05_ceo where c_no = (
	select f_ceo from jun05_franchise where f_seat = (
...
--	여러 번 정렬 하고 싶을 때.. => order by 컬럼명 (ASC/DESC), 컬럼명 (ASC/DESC), ...
select m_name, m_price from jun05_menu 
...
------------------------------------------------------------------------------
-- 제일 비싼 메뉴 이름, 가격
select m_name, m_price from jun05_menu where m_price in (
	select max(m_price) from jun05_menu
);

-- 최연장자 사장님의 이름, 생일 (최연장자 = 최소 생일 값)
select c_name, c_birthday from jun05_ceo where c_birthday in (
	select min(c_birthday) from jun05_ceo
);

-- 좌석 수 제일 적은 식당의 이름, 지점명, 좌석 수
select f_name, f_location, f_seat from jun05_franchise where f_seat in (
	select min(f_seat) from jun05_franchise
);
------------------------------------------------------------------------------
-- 홍콩반점 서초점을 운영하는 사람의 이름, 생일
select c_name, c_birthday from jun05_ceo where c_no = (
	select f_ceo from jun05_franchise where f_name = '홍콩반점' and f_location = '서초점'
);

-- '짜장'이 들어간 음식은 어디 가면 먹을 수 있나요? (식당 이름, 지점명)
select f_name, f_location from jun05_franchise where f_no in (
	select m_f_no from jun05_menu where m_name like '%짜장%'
);
------------------------------------------------------------------------------
-- 최연소 사장님네 가게 메뉴 이름, 가격
select m_name, m_price from jun05_menu where m_f_no in (
	select f_no from jun05_franchise where f_ceo in (
		select c_no from jun05_ceo where c_birthday in (
			select max(c_birthday) from jun05_ceo
		)
	)
);

-- 최길동이 운영하는 가게의 모든 메뉴 이름, 가격, 사장님 생일
select m_name, m_price, c_birthday from jun05_menu, jun05_ceo where m_f_no in (
	select f_no from jun05_franchise where f_ceo in (
		select c_no from jun05_ceo where c_name = '최길동'
	)
) and c_name = '최길동';

select m_name, m_price, (select c_birthday from jun05_ceo where c_no = 
	(select f_ceo from jun05_franchise where f_no = m_f_no)) c_birthday 
	from jun05_menu where m_f_no in (
		select f_no from jun05_franchise where f_ceo in (
			select c_no from jun05_ceo where c_name = '최길동'
		)
	);
...
-- 메뉴 이름을 가나다순으로 정렬해서 MENU 테이블 rownum 1 ~ 3까지 전체 정보를 조회
select rownum, m_no, m_name, m_price, m_f_no 
	from jun05_menu
	where rownum between 1 and 3	-- 이렇게 하면 order by 전에 rownum이 생성되어서
...
where rownum between 2 and 3;	-- 1번부터 조회해야 하기 때문에 오류 !

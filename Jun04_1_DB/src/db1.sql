...
select round(avg(s_price),1) from JUN03_SNACK where s_name like '%이%';

-- 크라운 과자거나, 프링글스 ~ 인거 이름, 제조사, 가격 조회
select s_name, s_maker, s_price from JUN03_SNACK where s_maker = '크라운' or s_name like '프링글스%';

-- 과자의 이름이 '새콤달콤'을 포함하거나 '프링글스'를 포함하는 과자 이름, 제조사, 가격
select s_name, s_maker, s_price from jun03_snack where s_name like '%새콤달콤%' or s_name like '%프링글스%';

-- like ~ in 구조는 아예 없다 ! 
select s_name, s_maker, s_price from jun03_snack where s_name like in ('%새콤달콤%', '%프링글스%'); -- 에러
...
select s_name, s_maker, s_price from JUN03_SNACK where regexp_like(s_name, '^새콤달콤|이$');
----------------------------------------------------------------------------------------
-- 집합 연산자
--	합집합
...
--	연산자 우선순위 (오른쪽이 최우선순위)
--	OR > AND > NOT > 비교연산자 > 괄호
select * from jun03_snack where s_maker = '크라운' and (s_price >= 3000 or s_price <= 1000);
---------------------------------------------------------------------------------------------
-- Group by : 그룹으로 묶어 연산해서 하나의 결과로 나타내는 것
-- 회사별 과자 평균가격, 최소가격, 최고가격
select s_maker, round(avg(s_price), 1), max(s_price), min(s_price)
	from jun03_snack
	group by s_maker;

-- Rollup 연산자
-- group by rollup(그룹 대상)
-- 컬럼에 대한 소계(subtotal)를 만들어 줌
-- group by 절의 컬럼 갯수가 n개라면, rollup을 썼을 때는 n+1개

-- 회사 이름, 회사별 과자 총 가격 합계와 모든 과자들의 가격 합계
select s_maker, sum(s_price) from jun03_snack
	group by rollup(s_maker);
	
-- s_maker가 null이면 '전체', 그게 아니면 s_maker 값	
select nvl(s_maker, '전체'), sum(s_price) from jun03_snack
	group by rollup(s_maker);

-- DECODE(컬럼명, 조건, 조건에 맞는 결과값, 조건에 맞지 않을 때 결과값)
select decode(s_maker, null, '전체', s_maker), sum(s_price) from jun03_snack
	group by rollup(s_maker);
...
-- 회사별 과자 평균 가격이 2000원 이상인 회사의 이름과 평균 가격 조회
select s_maker, round(avg(s_price), 1) from jun03_snack
	group by s_maker
...
--	각 회사의 평균 가격 내림차순 정렬
select s_maker, avg(s_price) from jun03_snack 

...
-- 데이터 조작어 (DML) 중에서 데이터를 넣게 하는 명령어인 INSERT와
...
--		: 오름차순 ASC(기본값이라 명시 안 해도 됨) / 내림차순 DESC
------------------------------------------------------------------------------------
-- *** 서브쿼리 (SubQuery)
...
-- 주질의(main query, 외부 질의)와 부속질의(subquery, 내부질의)로 구성됨
...
create table jun04_maker(
	m_name varchar2(3 char) primary key,
	m_address varchar2(40 char) not null,
	m_employee number(4) not null
);

insert into jun04_maker values('크라운', '용산', 120);
insert into jun04_maker values('해태', '천안', 100);
insert into jun04_maker values('농심', '동작', 70);
insert into jun04_maker values('롯데', '영등포', 150);
insert into jun04_maker values('캘러그', '동작', 50);

drop table jun04_maker cascade constraint purge;
